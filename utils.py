...
def L96PatchDataExtraction(xt,RMD):
  ...
  # extract subsequences
    dataTrainingNoNaN = image.extract_patches_2d(image=xt.values[0:5000:time_step,:],patch_size=(dT,GD96.parameters.J),max_patches=NbTraining)
    dataValNoNaN     = image.extract_patches_2d(image=xt.values[6000::time_step,:],patch_size=(dT,GD96.parameters.J),max_patches=NbVal)
    dataTestNoNaN     = image.extract_patches_2d(image=xt.values[8000::time_step,:],patch_size=(dT,GD96.parameters.J),max_patches=NbTest)
    ...
    for nn in range(0,dataVal.shape[1],time_step_obs):
        indrand = np.random.permutation(dataVal.shape[2])[0:int(0.5*dataVal.shape[2])]
        dataVal[:,nn,indrand] = dataValNoNaN[:,nn,indrand]
    for nn in range(0,dataTest.shape[1],time_step_obs):
  ...
  # Generate noisy observsation
    ...
    # Initial linear interpolation for each component
        ...
        for kk in range(0,mask_test.shape[1]):
            indt  = np.where( mask_test[ii,kk,:] == 1.0 )[0]
            indt_ = np.where( mask_test[ii,kk,:] == 0.0 )[0]
        if len(indt) > 1:
            indt_[ np.where( indt_ < np.min(indt)) ] = np.min(indt)
    ...
    # Initial linear interpolation for each component
        ...
        for kk in range(0,mask_val.shape[1]):
            indt  = np.where( mask_val[ii,kk,:] == 1.0 )[0]
            indt_ = np.where( mask_val[ii,kk,:] == 0.0 )[0]
        if len(indt) > 1:
            indt_[ np.where( indt_ < np.min(indt)) ] = np.min(indt)
    ...
    X_val_obs        = X_val_obs[:,:,0:dT]
    X_val            = X_val[:,:,0:dT]
    X_val_missing    = X_val_missing[:,:,0:dT]
    mask_val         = mask_val[:,:,0:dT]
